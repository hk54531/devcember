...
export const getFlagEmoji = (countryCode: string) =>
  String.fromCodePoint(
    ...[...countryCode.toUpperCase()].map(
      (x: string) => 0x1f1a5 + x.charCodeAt(0),
    ),
  );

// Add flag emoji to country data
export const countriesData = countries.map(i => ({
  ...i,
  flag: getFlagEmoji(i.code),
}));

interface CountryCodeSelectorState {
  openCountryCode: boolean;
...
interface CountryItem {
  ...
  label: string;
  phone: string;
  phoneLength: number | number[]; // Define phoneLength as number or number[]
  min?: number;
  max?: number;
  suggested?: boolean;
}

class CountryCodeSelector extends Component<{}, CountryCodeSelectorState> {
  constructor(props: {}) {
    super(props);
    this.state = {
      openCountryCode: false,
      searchCountry: '',
      phoneNumber: '',
      isFocused: false,
      selectedCountry: {
  ...
  handleToggleCountryCode = () => {
    this.setState({openCountryCode: !this.state.openCountryCode});
  };
  ...
  handleSearchCountry = (text: string) => {
  ...
  handleSelectCountry = (country: {
    ...
    console.log(country, '!!!');
    this.setState({selectedCountry: country, openCountryCode: false});
  };

  renderCountryItem = (item: any) => {
    return (
      <TouchableOpacity
  ...
  render() {
    const {openCountryCode, searchCountry, selectedCountry} = this.state;

    const filteredCountries = countriesData.filter(
      country =>
        country.label.toLowerCase().includes(searchCountry.toLowerCase()) ||
        country.phone.toLowerCase().includes(searchCountry.toLowerCase()),
    );
    ...
    return (
      <SafeAreaView style={{flex: 1, backgroundColor: 'white'}}>
        <View
          ...
          <TouchableOpacity
            onPress={this.handleToggleCountryCode}
            style={{
            ...
            <View style={{flexDirection: 'row'}}>
              <Text>{selectedCountry && `${selectedCountry.flag}`}</Text>
              <Text style={{marginLeft: 5}}>
                {selectedCountry && `+${selectedCountry.phone}`}
              </Text>
              <Ionicons
                style={{marginLeft: 5}}
                name={openCountryCode ? 'chevron-up' : 'chevron-down'}
                size={20}
                color="#000"
              />
            </View>
            ...
            style={[
              this.state.isFocused
                ? {
                    borderWidth: 5,
                    borderRadius: 10,
                    borderColor: 'rgba(225, 204, 255, 1)',
                    marginLeft: 15,
                  }
                : {},
            ]}>
        ...
        <Modal
          ...
          onRequestClose={() =>
            this.setState({
              openCountryCode: !this.state.openCountryCode,
            })
          }
          visible={openCountryCode}>
          <TouchableOpacity
            data-modal="modal1"
            testID="isModalTest"
            onPressOut={() =>
            ...
            <TouchableWithoutFeedback>
              <View
                ...
                <View
                  ...
                  />
                  <TextInput
                    onChangeText={e => this.handleSearchCountry(e)}
                    value={searchCountry}
                ...
                <View style={{backgroundColor: 'rgba(242, 240, 247, 1)'}}>
                  <Text
                    style={{
                      color: 'rgba(102, 102, 102, 1)',
                      fontFamily: 'Poppins',
